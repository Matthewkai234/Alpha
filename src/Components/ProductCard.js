...
function ProductCard({ product }) {
  ...
  const getTours = async () => {
    try {
      ...
      // Include other search params as needed

      const { data } = await axios.get(
        `http://localhost:4000/product?${params.toString()}&limit=1`
      );
      ...
      setTourStates(
        Object.fromEntries(data.products.map((tour) => [tour._id, false]))
      );
    } catch (error) {
      setIsLoading(false);
  ...
  useEffect(() => {
    getTours();
  }, [current]); // Add other dependencies if needed
...
{/* 
        ...
        <div className=" original-price">{product.price}$</div>
            ...
            <div className="discounted-price">dicount:{product.dicount}%</div>

            <div className="discounted-price">{product.finalPrice}$</div>            
        </div>
